...
export const useAppStore = defineStore({
    ...
    actions: {
        ...
        setViewAndWidth(route_name)
        {
            ...
            if(route_name.includes('apps.view')
                || route_name.includes('apps.form')
            ){
                this.view = 'list-and-item';
            }

            if(route_name.includes('apps.filters')){
        ...
        async getItemAfter(data, res)
        {
            if(data)
            {
                ...
                this.$router.push({name: 'apps.index',query:this.query});
            }
        ...
        async formActionAfter (data)
        {
            switch (this.form.action)
            {
                ...
                case 'create-and-clone':
                    this.item.id = null;
                    this.$router.push({name: 'apps.form',query:this.query,params: { id: null }});
                    await this.getFormMenu();
                    break;
                case 'trash':
                case 'restore':
        ...
        toForm()
        {
            this.item = vaah().clone(this.assets.empty_item);
            this.getFormMenu();
            this.$router.push({name: 'apps.form',query:this.query})
        },
        //---------------------------------------------------------------------
        toView(item)
        {
            ...
            this.$router.push({name: 'apps.view', params:{id:item.id},query:this.query})
        },
        //---------------------------------------------------------------------
        toEdit(item)
        {
            ...
            this.$router.push({name: 'apps.form', params:{id:item.id},query:this.query})
        },
        //---------------------------------------------------------------------
        isListView()
        {
        ...
        getItemTenants(data) {
            this.$router.push({ name: 'tenantapps.index', query: { search_by: 'app', filter: {q: data.name }} });
        }
    },
    });
