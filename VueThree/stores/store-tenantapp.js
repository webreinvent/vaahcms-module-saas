...
let model_namespace = 'VaahCms\\Modules\\Saas\\Models\\TenantAppsV3';


let base_url = document.getElementsByTagName('base')[0].getAttribute("href");
let ajax_url = base_url + "/saas/tenantapp";
...
export const useTenantAppStore = defineStore({
    ...
    actions: {
        ...
        setViewAndWidth(route_name)
        {
            ...
            if(route_name.includes('tenantapp.view')
                || route_name.includes('tenantapp.form')
            ){
                this.view = 'list-and-item';
            }

            if(route_name.includes('tenantapp.filters')){
        ...
        async getItemAfter(data, res)
        {
            if(data)
            {
                ...
                this.$router.push({name: 'tenantapp.index',query:this.query});
            }
        ...
        async formActionAfter (data)
        {
            switch (this.form.action)
            {
                ...
                case 'create-and-clone':
                    this.item.id = null;
                    this.$router.push({name: 'tenantapp.form',query:this.query,params: { id: null }});
                    await this.getFormMenu();
                    break;
                case 'trash':
                case 'restore':
        ...
        toForm()
        {
            this.item = vaah().clone(this.assets.empty_item);
            this.getFormMenu();
            this.$router.push({name: 'tenantapp.form',query:this.query})
        },
        //---------------------------------------------------------------------
        toView(item)
        {
            ...
            this.$router.push({name: 'tenantapp.view', params:{id:item.id},query:this.query})
        },
        //---------------------------------------------------------------------
        toEdit(item)
        {
            ...
            this.$router.push({name: 'tenantapp.form', params:{id:item.id},query:this.query})
        },
        //---------------------------------------------------------------------
        isListView()
        {
