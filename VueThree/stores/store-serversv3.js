...
let model_namespace = 'VaahCms\\Modules\\Saas\\Models\\ServerV3';
...
export const useServerV3Store = defineStore({
    id: 'serversv3',
    state: () => ({
        base_url: base_url,
        ajax_url: ajax_url,
    ...
    actions: {
        ...
        setViewAndWidth(route_name)
        {
            ...
            if(route_name.includes('serversv3.view')
                || route_name.includes('serversv3.form')
            ){
                this.view = 'list-and-item';
            }

            if(route_name.includes('serversv3.filters')){
        ...
        async getItemAfter(data, res)
        {
            if(data)
            {
                ...
                this.$router.push({name: 'serversv3.index',query:this.query});
            }
        ...
        async formActionAfter (data)
        {
            switch (this.form.action)
            {
                ...
                case 'create-and-clone':
                    this.item.id = null;
                    this.$router.push({name: 'serversv3.form',query:this.query,params: { id: null }});
                    await this.getFormMenu();
                    break;
                case 'trash':
                case 'restore':
        ...
        toForm()
        {
            this.item = vaah().clone(this.assets.empty_item);
            this.getFormMenu();
            this.$router.push({name: 'serversv3.form',query:this.query})
        },
        //---------------------------------------------------------------------
        toView(item)
        {
            ...
            this.$router.push({name: 'serversv3.view', params:{id:item.id},query:this.query})
        },
        //---------------------------------------------------------------------
        toEdit(item)
        {
            ...
            this.$router.push({name: 'serversv3.form', params:{id:item.id},query:this.query})
        },
        //---------------------------------------------------------------------
        isListView()
        {
...
// Pinia hot reload
if (import.meta.hot) {
    import.meta.hot.accept(acceptHMRUpdate(useServerV3Store, import.meta.hot))
}
