...
export const useTenantAppStore = defineStore({
    id: 'tenantapps',
    state: () => ({
        base_url: base_url,
        ajax_url: ajax_url,
    ...
    actions: {
        ...
        setViewAndWidth(route_name)
        {
            ...
            if(route_name.includes('tenantapps.view')
                || route_name.includes('tenantapps.form')
            ){
                this.view = 'list-and-item';
            }

            if(route_name.includes('tenantapps.filters')){
        ...
        async getItemAfter(data, res)
        {
            if(data)
            {
                ...
                this.$router.push({name: 'tenantapps.index',query:this.query});
            }
        ...
        async formActionAfter (data)
        {
            switch (this.form.action)
            {
                ...
                case 'create-and-clone':
                    this.item.id = null;
                    this.$router.push({name: 'tenantapps.form',query:this.query,params: { id: null }});
                    await this.getFormMenu();
                    break;
                case 'trash':
                case 'restore':
        ...
        toForm()
        {
            this.item = vaah().clone(this.assets.empty_item);
            this.getFormMenu();
            this.$router.push({name: 'tenantapps.form',query:this.query})
        },
        //---------------------------------------------------------------------
        toView(item)
        {
            ...
            this.$router.push({name: 'tenantapps.view', params:{id:item.id},query:this.query})
        },
        //---------------------------------------------------------------------
        toEdit(item)
        {
            ...
            this.$router.push({name: 'tenantapps.form', params:{id:item.id},query:this.query})
        },
        //---------------------------------------------------------------------
        isListView()
        {
...
// Pinia hot reload
if (import.meta.hot) {
    import.meta.hot.accept(acceptHMRUpdate(useTenantAppStore, import.meta.hot))
}
